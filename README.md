...
- `.claude/commands/ei-*.md` — slash commands públicos (`/ei-cria-cliente`, `/ei-ajustes`, `/ei-update`). Comandos de mantenedor (`/ei-edit`, `/ei-review`, `/ei-ctx`) **não são distribuídos** via `npx ei-prompt` — ficam no clone do repo source.
