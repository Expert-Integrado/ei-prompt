...
- **Pipeline via hook (Stop event):** ao fim do fan-out de editores no Passo 5, o hook `.claude/hooks/post-ajustes-fanout.sh` (registrado no `.claude/settings.json` no evento `Stop`) detecta o sentinela `<ei-ajustes-round id=...>` no transcript e injeta `reason` (schema correto do Stop event — não `additionalContext`) instruindo o main Claude a emitir `<ei-ajustes-round-consumed id=...>` e prosseguir ao Passo 6. Protocolo sentinela ↔ consumed é stateless (zero arquivo de estado), idempotente, e re-dispara em CADA novo dispatch (retry PARL-04, correção REVW-04). Fallback é o **ESTADO PADRÃO**: se o hook não acionar (desabilitado / com bug / settings sem o bloco Stop), o `/ei-ajustes` continua manual como Phase 4 e adiciona a nota D-17 no resumo final. Hook antigo `.claude/hooks/post-scaffolder-review.sh` (SubagentStop, `client-scaffold-fill`) ganha guarda silenciosa que detecta o sentinela e sai exit 0 (D-11 — coexistência sem conflito). Detalhe em `.claude/commands/ei-ajustes.md` (Passo 5 REGRA INVIOLÁVEL HOOK-02 + Passo 6 REGRA INVIOLÁVEL HOOK-02).
